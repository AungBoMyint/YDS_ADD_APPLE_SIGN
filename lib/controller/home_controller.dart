...
class HomeController extends GetxController {
  ...
  RxList<DrivingLicenceForm> drivingLicenceFormList =
  ...
  final Rxn<AuthUser?> currentUser = Rxn<AuthUser?>(AuthUser(
    id: "gustID",
    emailAddress: "guest@email.com",
    userName: "Guest",
    image: userImage,
    points: 0,
    token: "gustToken",
  ...
  void watchOnlyIfAdmin() {
    ...
    _database.watchWithDateTime(courseFormCollection).listen((event) {
      //CourseForm
      enrollList.value =
          event.docs.map((e) => CourseForm.fromJson(e.data())).toList();
    });
    _database.watchWithDateTime(drivingLicenceCollection).listen((event) {
      //DrivingLicenceForm
      drivingLicenceFormList.value =
          event.docs.map((e) => DrivingLicenceForm.fromJson(e.data())).toList();
    });
    _database.watchWithDateTime(carLicenceCollection).listen((event) {
      //CarLicenceForm
      carLicenceFormList.value =
          event.docs.map((e) => CarLicenceForm.fromJson(e.data())).toList();
    });
    _database.watchWithDateTime(purchaseCollection).listen((event) {
      //PurchaseModelForm
      if (event.docs.isNotEmpty) {
        ...
        memicList.forEach((element) {
          final matchEnrollList =
              enrollList.where((p0) => p0.userID == element.userId);
          final matchCarList =
              carLicenceFormList.where((p0) => p0.userId == element.userId);
          final matchDrivingList =
              drivingLicenceFormList.where((p0) => p0.userId == element.userId);
