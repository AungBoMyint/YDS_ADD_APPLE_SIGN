...
import 'package:hammies_user/data/enum.dart' hide Price;
...
class CourseFormController extends GetxController {
  ...
  submitForm({
      ...
      if (_homeController.bankSlipImage.isNotEmpty) {
        await _firebaseStorage
          ...
          await snapshot.ref.getDownloadURL().then((value) async {
            ...
            _database
                .write(courseFormCollection,
                    path: uuId,
                    data: courseModel.copyWith(bankSlipImage: value).toJson())
        ...
        await Future.delayed(const Duration(seconds: 0));
        _database
            .write(courseFormCollection, path: uuId, data: courseModel.toJson())
            .then((value) => Api.sendPushToAdmin(
                "သင်တန်းအပ်ခြင်း",
                "🧑အမည်:${inputMap["name"]?.text ?? ""}\n"
                    "🏠လိပ်စာ: ${inputMap["address"]?.text ?? ""}\n"
    ...
    _homeController.changePaymentOptions(PaymentOptions.None);
    _homeController.setBankSlipImage('');
    log("****Success ");
  }
  ...
  void onInit() {
    ...
    _database.readCollection(coursePriceCollection).then((value) {
      if (value.docs.isNotEmpty) {
        priceList.value =
            value.docs.map((e) => Price.fromJson(e.data())).toList();
