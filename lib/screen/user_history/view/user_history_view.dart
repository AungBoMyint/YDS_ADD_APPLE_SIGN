        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            child: ListTile(
              ...
              subtitle: Text("စတက်သည့်ရက် => ${getDate(course.initialDay)}"),
              trailing: Text("${getDate(course.dateTime)}"),
            ),
          );
        },
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      if (_controller.drivingLicenceFormList.isEmpty) {
        return const Center(
          child: Text("No driving licence yet!."),
        );
      }
      return Obx(() {
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              child: ListTile(
                onTap: () =>
                    Get.to(() => DLHDetail(drivingLicenceForm: course)),
                title: Text(
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              child: ListTile(
                onTap: () =>
                    Get.to(() => PurchaseModelDetail(purchaseModel: course)),
                title: Text(
                  "Delivery Fees => ${course.deliveryTownshipInfo?[0] ?? "0"}",
                ),
                trailing: Text("${getDate(DateTime.parse(course.dateTime))}"),
              ),
            );
          },
        );
